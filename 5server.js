...
// Returns null if safe, or a reason string if rejected (so callers can log specifics).
function checkModSafety(filePath, newContent) {
  if (!fs.existsSync(filePath)) return null;
  const original = fs.readFileSync(filePath, 'utf-8');
  const minLength = original.length * 0.4;
  if (newContent.length < minLength) {
    return `new content is ${newContent.length} chars vs original ${original.length} chars (below the ${Math.round(minLength)}-char safety floor)`;
  }
  return null;
}
...
async function runOptimizationJob(jobId, repoUrl, mergeToMain) {
  ...
  try {
    ...
    try {
      const response = await ai.models.generateContent({
        ...
        config: {
          ...
          maxOutputTokens: 60000, // leave headroom under the model's ~64k cap since full file contents are echoed back
        },
      });

      const text = response.text;
      if (!text) {
        // response.text comes back undefined if the model was cut off before producing
        // output (commonly: ran out of output tokens because too many/large HTML files
        // were sent, so it never got past "thinking") or if the response was blocked.
        const finishReason = response.candidates?.[0]?.finishReason || 'unknown';
        const blockReason = response.promptFeedback?.blockReason;
        throw new Error(
          `Gemini returned no text (finishReason: ${finishReason}` +
          (blockReason ? `, blockReason: ${blockReason}` : '') +
          `). This usually means the output was cut off by the token limit -- try pointing ` +
          `this at a repo with fewer/smaller HTML files, or fewer files will be sent at once.`
        );
      }

      job.log(`Received AI response (${text.length} chars).`);
      data = JSON.parse(text); // schema-enforced, so this should always be valid JSON if we got here
      job.log(`AI proposed ${data.modifications.length} file modification(s) before filtering.`);
    } catch (e) {
    ...
    for (const mod of data.modifications) {
      ...
      const unsafeReason = checkModSafety(filePath, mod.content);
      if (unsafeReason) {
        job.log(`Skipped ${mod.path} (safety check: ${unsafeReason})`);
        skipped++;
        continue;
      }
...
app.post('/api/optimize', requireAuth, (req, res) => {
  ...
  jobs[jobId] = {
    ...
    log(msg) {
      this.logs.push(msg);
      console.log(`[job ${jobId}] ${msg}`); // also print to server console/Render logs, independent of the frontend
    },
  };
